...
let debugMode = true; // disable in production
...
async function loadConversations(status = "open") {
    ...
    // Show loader, hide list and errors
    ...
    try {
        const response = await fetch(
            `/admin/customer-support/conversations?status=${status}`,
            {
                headers: {
                    "X-Requested-With": "XMLHttpRequest",
                    "X-CSRF-TOKEN":
        ...
        debugLog("info", "Conversations loaded", {
    ...
    listEl.innerHTML = conversations
        .map((conv) => {
            ...
            const lastActivity = conv.last_activity_at
                ? formatTime(conv.last_activity_at * 1000)
                : "";
            ...
            return `<div class="conversation-item${
                ...
                conv.id
            }" onclick="selectConversation(event, ${conv.id})">
            <div class="d-flex justify-content-between align-items-start">
                <div class="flex-grow-1">
                    <h6 class="mb-1">${senderName}</h6>
                    ${
                        senderEmail
                            ? `<small class="text-muted">${senderEmail}</small><br>`
                            : ""
                    }
                    <p class="mb-1 text-muted small">${truncate(
                        lastMsgContent,
                        50
                    )}</p>
...
async function loadMessages(conversationId) {
    ...
    try {
        const response = await fetch(
            `/admin/customer-support/conversations/${conversationId}/messages`,
            {
                headers: {
                    "X-Requested-With": "XMLHttpRequest",
                    "X-CSRF-TOKEN":
        ...
        // Track processed message ids and last message id
...
function displayMessages(messages) {
    ...
    if (!messages || messages.length === 0) {
        messagesEl.innerHTML =
            '<div class="text-center py-4"><p class="text-muted">No messages in this conversation</p></div>';
        return;
    }
    ...
    const messagesHTML = messages
        .map((msg) => {
            ...
            <div class="mb-1">
                ...
                <span class="message-time float-right">${formatTime(
                    msg.created_at * 1000
                )}</span>
            </div>
            <div>${
                msg.content || msg.processed_message_content || "[No content]"
            }</div>
            ${
                msg.status
...
function appendMessage(message) {
        ...
        typeof message.message_type === "string"
            ? message.message_type === "incoming" ||
              message.message_type === "0"
                ? 0
                : 1
            : parseInt(message.message_type) || 0;
    ...
    // Format timestamp reliably
    let timestamp;
    if (typeof message.created_at === "string") {
        timestamp = new Date(message.created_at);
    } else if (typeof message.created_at === "number") {
        timestamp = new Date(
            message.created_at < 9999999999
...
async function sendQuickReply(event) {
    ...
    try {
        const response = await fetch(
            `/admin/customer-support/conversations/${selectedConversationId}/messages`,
            {
                method: "POST",
                headers: {
                ...
                case "message_created": {
                    ...
                    if (message && message.id) {
                        if (convId == selectedConversationId) {
                            debugLog(
                                "debug",
                                "Message is for selected conversation, appending"
                            );
                            appendMessage(message);
                        } else {
                            debugLog(
                                "debug",
                                "Message is for different conversation",
                                convId
                            );
                        }
                    } else {
                    ...
                    loadConversations(); // Update conversations list to reflect latest
                    loadStats();
                    break;
                }
...
window.debugSSE = {
    ...
    clearCache: async () => {
        try {
            const response = await fetch(
                "/admin/customer-support/clear-cache",
                {
                    method: "POST",
                    headers: {
                        "X-Requested-With": "XMLHttpRequest",
