...
@section('content')
    <main class="main-content" id="user-content">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h3 class="fw-bold">{{ __('welcome.key_703') }}</h3>
                ...
                <form action="{{ route('admin.careers.store') }}" method="POST">
                    ...
                    <div class="d-flex justify-content-between col-md-12 mb-3">
                        ...
                        <div class="col-md-5 mb-3">
                            ...
                            <select name="type" id="job-type" class="form-control @error('type') is-invalid @enderror"
                                ...
                                <option value="full_time">{{ __('welcome.key_708') }}</option>
                                <option value="part_time">{{ __('welcome.key_709') }}</option>
                                <option value="contract">{{ __('welcome.key_710') }}</option>
                            </select>
                            @error('type')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                    ...
                    <div class="col-md-5 mb-3">
                        ...
                        <select name="is_active" id="job-status"
                            ...
                            <option value="1">{{ __('welcome.key_713') }}</option>
                            <option value="0">{{ __('welcome.key_714') }}</option>
                        </select>
                        @error('is_active')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
