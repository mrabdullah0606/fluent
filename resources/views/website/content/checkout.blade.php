...
<!-- <script>
  document.getElementById('stripeCheckoutBtn').addEventListener('click', function (e) {
    ...
    // Example order items (replace with real dynamic data from backend or DOM if needed)
    const orderItems = [
      { name: "Premium Coaching", price: 10000, quantity: 1 } // price in cents (e.g., $100)
    ];

    // Total amount (also in cents)
    const amount = 10300;
...
<!-- <script>
  document.getElementById('stripe-pay-button').addEventListener('click', function (e) {
    ...
    // Order Summary - Replace with dynamic values if needed
    const orderItems = [
      { name: 'Product A', quantity: 2, price: 1500 },
      { name: 'Product B', quantity: 1, price: 2500 },
    ];
    const totalAmount = 5500; // in cents

    const orderInput = document.createElement('input');
    orderInput.type = 'hidden';
    orderInput.name = 'order_items';
