<div class="settings-card">
    ...
    @for ($i = 1; $i <= 3; $i++)
        @php
            // Safely fetch this package (or empty array if not exists)
            $package = $packages[$i] ?? [];
        @endphp
        ...
        <div class="package-container">
            ...
            <div class="row g-2">
                ...
                <div class="col-md-4">
                    ...
                    <input type="number"
                        name="packages[{{ $i }}][number_of_lessons]"
                        id="package_{{ $i }}_lessons"
                        class="form-control"
                        min="1"
                        value="{{ old('packages.' . $i . '.number_of_lessons', $package['number_of_lessons'] ?? '') }}"
                        required>
                </div>

                {{-- Duration --}}
                <div class="col-md-4">
                    <label class="form-label small" for="package_{{ $i }}_duration">Duration per Lesson (mins)</label>
                    <select name="packages[{{ $i }}][duration_per_lesson]"
                        id="package_{{ $i }}_duration"
                        class="form-select" required>
                        @foreach ([60] as $duration)
                            <option value="{{ $duration }}"
                                {{ old('packages.' . $i . '.duration_per_lesson', $package['duration_per_lesson'] ?? '') == $duration ? 'selected' : '' }}>
                                {{ $duration }} mins
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-4">
                    ...
                    <input type="number"
                        ...
                        id="package_{{ $i }}_price"
                        class="form-control"
                        step="0.01"
                        min="0"
                        value="{{ old('packages.' . $i . '.price', $package['price'] ?? '') }}"
                        readonly>
                </div>
            </div>
            ...
            <div class="form-check form-switch position-absolute top-0 end-0 mt-2 me-3">
                <input class="form-check-input bg-warning border"
                    type="checkbox"
                    name="packages[{{ $i }}][is_active]"
                    id="package_{{ $i }}_active"
                    value="1"
                    {{ old('packages.' . $i . '.is_active', $package['is_active'] ?? true) ? 'checked' : '' }}>
