    ...
    <script>
        ...
        async function addTimeSlots() {
            ...
            try {
                ...
                console.log('Submitting time slots:', {
                    date: dateString,
                    start_time: start24, // Send 24-hour format
                    end_time: end24 // Send 24-hour format
                });
                ...
                const response = await apiRequest(`${API_BASE}/store`, {
                    ...
                    body: JSON.stringify({
                        date: dateString,
                        start_time: start24, // Changed: send 24-hour format
                        end_time: end24 // Changed: send 24-hour format
                    })
                });
        ...
        // Add these additional functions to your JavaScript
        ...
        function updateTimePreview() {
            try {
                ...
                if (!startTimeInput || !endTimeInput || !startAmPmSelect || !endAmPmSelect || !previewText) {
                    return;
                }

                const startTime = startTimeInput.value;
                const endTime = endTimeInput.value;
                const startAmPm = startAmPmSelect.value.toUpperCase();
                const endAmPm = endAmPmSelect.value.toUpperCase();

                if (startTime && endTime) {
                    // Convert to 12-hour format for display
                    const startDisplay = formatTimeForDisplay(startTime, startAmPm);
                    const endDisplay = formatTimeForDisplay(endTime, endAmPm);
        ...
        function formatTimeForDisplay(time, ampm) {
            const [hours, minutes] = time.split(':');
        ...
        // Updated resetModalForm function with event listeners
        function resetModalForm() {
            try {
                const startTime = document.getElementById('startTime');
                const endTime = document.getElementById('endTime');
                ...
                if (startAmPm) {
                    startAmPm.value = 'am';
                    startAmPm.removeEventListener('change', updateTimePreview);
                    startAmPm.addEventListener('change', updateTimePreview);
                }
                if (endAmPm) {
                    endAmPm.value = 'pm';
                    endAmPm.removeEventListener('change', updateTimePreview);
                    endAmPm.addEventListener('change', updateTimePreview);
                }
        ...
        // Enhanced openAddTimeSlotsModal function
        function openAddTimeSlotsModal() {
            try {
                const modalElement = document.getElementById('addTimeSlotsModal');
                if (!modalElement) {
                ...
                modalElement.addEventListener('shown.bs.modal', function() {
                    ...
                    if (startTime) startTime.addEventListener('change', updateTimePreview);
                    if (endTime) endTime.addEventListener('change', updateTimePreview);
                    if (startAmPm) startAmPm.addEventListener('change', updateTimePreview);
                    if (endAmPm) endAmPm.addEventListener('change', updateTimePreview);
        ...
        function validateTimeInputs() {
            ...
            console.log('Validation - Original inputs:', {
                startTime,
                endTime,
                startAmPm,
                endAmPm
            });
            console.log('Validation - Converted times:', {
                start24,
                end24
            });
        ...
        function debugModal() {
            ...
            console.log('Modal element:', document.getElementById('addTimeSlotsModal'));
            console.log('Start time element:', document.getElementById('startTime'));
            console.log('End time element:', document.getElementById('endTime'));
            console.log('Start AM/PM element:', document.getElementById('startAmPm'));
            console.log('End AM/PM element:', document.getElementById('endAmPm'));
        ...
        // Add this to your existing addTimeSlots function to replace the validation part
        async function addTimeSlots() {
            const addBtn = document.getElementById('addSlotsBtn');
            if (!addBtn) {
                console.error('Add slots button not found');
            ...
            try {
                ...
                const timeInputs = validateTimeInputs();
                ...
                console.log('Final submission data:', {
                    date: dateString,
                    start_time: timeInputs.start24,
                    end_time: timeInputs.end24,
                    original_inputs: {
                        start: timeInputs.startTime,
                        end: timeInputs.endTime,
                        start_ampm: timeInputs.startAmPm,
                        end_ampm: timeInputs.endAmPm
                    }
                });
